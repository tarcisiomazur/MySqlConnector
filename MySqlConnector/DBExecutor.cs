...
namespace MySqlConnector.MySqlConnector
{
    public static class DbExecutor
    {
        ...
        private static string BuildFkField(Field key, ManyToOne manyToOne)
        {
            var str = $"{manyToOne.FieldFkName(key)} {MySqlProtocol.GetSqlFieldType(key)} ";
            ...
            List<KEY_TABLE_SCHEMA> keys)
        {
            ...
            {
                var fName = BuildFkField(field, manyToOne);
                var sch = tableCols.FirstOrDefault(cts => cts.COLUMN_NAME.SQLEquals(fName));
                fieldChanges += (sch == default? "ADD COLUMN " : "MODIFY COLUMN ") + fName;
            }
            ...
            if (tableCols.Count == 0)
                ...
                query += "ALTER TABLE @schema.@table @fieldChanges," +
                         "ADD CONSTRAINT @fk_name FOREIGN KEY (@onlykeys) REFERENCES @schema.@ref_table(@ref_field);";

            var cmd = MysqlManager.CreateTransaction();
            cmd.CommandText = query;
            cmd.SetParameter("@schema", table.Schema);
            cmd.SetParameter("@table", table.SqlName);
            cmd.SetParameter("@onlykeys", string.Join(",", manyToOne.Links.Select(manyToOne.FieldFkName)));
            cmd.SetParameter("@keysAndType", string.Join(",", manyToOne.Links.Select(key=>BuildFkField(key,manyToOne))));
            cmd.SetParameter("@fk_name", manyToOne.FkName);
            cmd.SetParameter("@ref_table", manyToOne.TableReferenced.SqlName);
            cmd.SetParameter("@ref_field", string.Join(",", manyToOne.Links.Select(field=>field.SqlName)));
